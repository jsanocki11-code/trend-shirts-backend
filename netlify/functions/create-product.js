...
exports.handler = async function (event) {
  ...
  try {
    ...
    // Verify the write actually persisted, retrying a few times to allow
    // for JSONBin's propagation delay rather than failing on the first check
    let verifyShirts = [];
    for (let attempt = 1; attempt <= 3; attempt++) {
      await new Promise((resolve) => setTimeout(resolve, attempt * 800));
      debugLog.push("Verify attempt " + attempt + "...");
      const verifyRes = await fetch(binUrl + "/latest", {
        headers: { "X-Master-Key": JSONBIN_API_KEY },
      });
      const verifyData = await verifyRes.json();
      verifyShirts = (verifyData.record || {}).shirts || [];
      debugLog.push("Attempt " + attempt + " shows " + verifyShirts.length + " shirts.");
      if (verifyShirts.length === record.shirts.length) break;
    }

    if (verifyShirts.length !== record.shirts.length) {
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({
          error: "Save appeared to succeed but verification read-back doesn't match after retries. This points to a JSONBin issue, not our code.",
          expected: record.shirts.length,
          actual: verifyShirts.length,
          productUrl,
          debugLog,
        }),
      };
    ...
    debugLog.push("Verified — save confirmed.");
    return {
      statusCode: 200,
      headers,
